    ...
    <div className="overflow-hidden py-1.5">
      ...
      >
        {doubled.map((phrase, i) => (
          <span
            key={i}
            className="inline-flex shrink-0 items-center rounded-full px-4 py-2 text-sm"
            style={{
    ...
    <div className="flex flex-col gap-3">
      <form onSubmit={handleSend}>
        <div
          ...
          style={{
            background: "#1a1a1a",
            border: `1px solid ${focused ? "rgba(207,199,189,0.5)" : "rgba(207,199,189,0.12)"}`,
            boxShadow: focused
        ...
        >
          <input
            ...
            disabled={sending}
            className="flex-1 bg-transparent text-sm text-[var(--foreground)] placeholder-[var(--text-muted)] outline-none disabled:opacity-50"
          />
          <button
            type="submit"
